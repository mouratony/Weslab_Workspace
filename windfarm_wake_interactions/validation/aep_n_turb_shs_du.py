...
def n_turbine_AEP_Plot(site1, wt_x=None, wt_y=None, figsave=False,
    ...
    # Upstream boundary (Sheringham Shoal)
    filepath = "DigitizeLayers/Europe/sheringham_shoal.geojson"
    eastings, northings = get_only_boundary(filepath)
    ...
    for cd in tqdm(capacity_densities, desc="Capacity density steps [MW/km²]"):
        ...
        spacing = spacing_D0
        up_x, up_y = grid_WTposition_generator(boundary_points, SWT_36_107(), n_turbs, spacing=spacing)
    ...
    fig_layout.suptitle("Sheringham Shoal layouts", fontsize=title_fontsize)
    fig_layout.tight_layout()
    if figsave:
        fig_layout.savefig(f"shs_du_layout_grid_{_ID_}.png", dpi=600)
    ...
    plt.title("Upstream turbine count impact on Dudgeon AEP", fontsize=title_fontsize)
    plt.tick_params(axis='both', which='major', labelsize=axis_tick_size)
    if figsave:
        plt.savefig(f"shs_du_vs_N_{_ID_}.png", dpi=600)
