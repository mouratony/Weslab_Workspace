...
with open('/Users/Antonio/Desktop/Research_Projects/Wind_Energy_lab_V1/Wind_Farms_Wake_Interactions/Examples/les_fig6.csv', 'r') as f:
    data_array = np.loadtxt(f, delimiter=',')

with open('/Users/Antonio/Desktop/Research_Projects/Wind_Energy_lab_V1/Wind_Farms_Wake_Interactions/Examples/les_overlay_fig6.csv'
          , 'r') as f:
    _data_array = np.loadtxt(f, delimiter=',')

with open('/Users/Antonio/Desktop/Research_Projects/Wind_Energy_lab_V1/Wind_Farms_Wake_Interactions/Examples/mesoscale_fig6.csv'
          , 'r') as f:
...
#Case 1: ws=7.24 m/s, wd=219
ws = 7.24  #m/s
wd = 219
ws_min = 3.0 #m/s
#Case 2: ws=8.83 m/s, wd=208
# ws = 8.83  #m/s
# wd = 208
# ws_min = 3.0 #m/s
...
#                             plot_colorbar=False, plot_windturbines=False)
# # add your own colorbar with extend markers
# cb = plt.colorbar(cs, extend='both')
# cb.set_label('wind speed (m/s)')
# plt.show()

alpha_down = np.deg2rad((wd + 180) % 360)  # 43° in radians
# perpendicular cross-section orientation
alpha_cross = np.deg2rad((wd - 90) % 360)  # 133° in radians
...
y_center = y0 - 2000 #+ 7400.0 #* np.cos(alpha_down)
...
y_transect = [y_center] * len(s) #+ s * np.cos(alpha_cross)
...
cs, cbar = _plot_wake(flow_map, ax=ax, levels=200, vmin=ws_min, vmax=ws, cmap='viridis', 
                      cbar_height= 5,
                       ...
                       cbar_ticks=[3,4,5,6,7] #case 1 
                       # cbar_ticks=[6, 8] #case 2 
                      )

# overlay transect and midpoint
ax.plot(x_transect, y_transect, 'k.', markersize=1.7)
ax.scatter([x_center], [y_center], color='r', zorder=10)
...
ax.set_xlim(xmin - 5000, xmax + 10000)
ax.set_ylim(ymin - 5000, ymax + 10000)
plt.tight_layout()
...
for name, (model, color, ls) in deficitModels_1.items():
    ...
    axes[0].plot([0, 0], [0, 7.5], 'k', ls='--', lw=1)
...
for name, (model, color, ls) in deficitModels_2.items():
    ...
    axes[1].plot([0, 0], [0, 7.5], 'k', ls='--', lw=1)
    axes[1].set_ylim(5.0, 7.5)
