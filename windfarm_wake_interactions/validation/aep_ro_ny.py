...
def main():
    ...
    # Ro_wt_x, Ro_wt_y = random_WTposition_generator(10, boundary_points, Haliade_X_12(), spacing=9)
    ...
    point_dist_x = np.array([6.6938e05,6.7257e05])
    point_dist_y = np.array([6.04721e06, 6.04721e06])

    all_x = np.r_[Ro_wt_x, Ny_wt_x]
    all_y = np.r_[Ro_wt_y, Ny_wt_y]
    ...
    # # plt.scatter(all_x, all_y, c=['k*']*len(Ro_wt_x) + ['r*']*len(Ny_wt_x), s=100, label='Wind Turbines', edgecolors='k')
    ...
    #-15000
    # 100000
    shift_values = range(0, 50000, 2500)
    ...
    plt.tight_layout()
    plt.savefig(f"AEP_diff_Ro&Nyro_{_ID_}", dpi=1000)
