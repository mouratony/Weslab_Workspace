...
class test_turbine(GenericWindTurbine):
    def __init__(self, D = 107, H = 80, P = 3600):
        """
        Parameters
        ----------
        D: Diameter (m)
...
def rotorDiameter_AEP_Plot(site, wt_x, wt_y, neighbour_x, neighbour_y, 
    ...
    for D in tqdm(rotor_diameters, desc="Processing Rotor Diameter"):
        ...
        windTurbines = WindTurbines.from_WindTurbine_lst([SWT_60_154(), w_turbine])
    ...
    plt.tight_layout()
    if figsave:
        plt.savefig(f"rotorDiameter_AEP_diff_Du&Shs_{_ID_}.png", dpi=1000)
    else:
        plt.show()
...
def ratedPower_AEP_Plot(site, wt_x, wt_y, neighbour_x, neighbour_y, 
    ...
    plt.tight_layout()
    if figsave:
        plt.savefig(f"RatedPower_AEP_diff_Du&SHS_{_ID_}.png", dpi=1000)
    else:
        plt.show()
...
def main():
    ...
    neighbour_x = neighbour_x_i + 5000 # 9 km apart (Euclidean Distance)
    neighbour_y = neighbour_y_i + 5000
